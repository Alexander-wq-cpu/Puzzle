...
public class Game extends JFrame {
    ...
    public void initUI(){
        ...
        heigth = resized.getHeight();

        add(panel,BorderLayout.CENTER);

        for (int i = 0; i<4; i++){
            for(int j =0;j<3;j++){
                image = createImage(new FilteredImageSource(resized.getSource(),new CropImageFilter(j*width/3,i*heigth/4,width/3,heigth/4)));
    ...
    private BufferedImage loadImage() throws IOException {
        BufferedImage bimg = ImageIO.read(new File("beautiful-natural-landscape.jpg"));
        return bimg;
    }
